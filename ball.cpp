...
void Ball::SetAsPlayer()
{
	...
	// Set position of player in middle of the screen
	m_pSprite->SetX(sm_fBoundaryWidth / 2.0f);
	m_pSprite->SetY(sm_fBoundaryHeight / 2.0f);
...
{
	return (static_cast<float>(m_pSprite->GetWidth()) / 2.0f);
}
...
void Ball::Shrink()
{
	// If ball is alive, half the current scale of the (player) ball
	if (m_bAlive)
	{
		float currentScale = m_pSprite->GetScale();
		m_pSprite->SetScale(currentScale - 0.025);
	}
}

void Ball::Enlarge()
{
	// If ball is alive, double the current scale of the (player) ball
	if (m_bAlive)
	{
		float currentScale = m_pSprite->GetScale();
		m_pSprite->SetScale(currentScale + 0.025);
