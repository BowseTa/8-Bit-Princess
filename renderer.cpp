...
bool Renderer::Initialise(bool windowed, int width, int height)
{
	...
	if (mFMODSystem)
	{
		mFMODSystem->init(512, FMOD_INIT_NORMAL, 0);
	}
...
void Renderer::Clear()
{
	...
	ImGui_ImplSDL2_NewFrame(); // code on slides had m_pWindow in brackets but this takes zero args
	ImGui::NewFrame();
}
...
void Renderer::DrawSprite(Sprite& sprite)
{
	...
	Matrix4 world;
	SetIdentity(world);
	world.m[0][0] = cosf(angleInRadians) * (sprite.GetScaleX() * sizeX);
	world.m[0][1] = -sinf(angleInRadians) * (sizeX);
	// Inverted the Y rotation matrix below from positive sinf and cosf to negative sinf and cosf and it fixed issue
...
void Renderer::DrawAnimatedSprite(AnimatedSprite& sprite, int frame)
{
	...
	Matrix4 world;
	SetIdentity(world);
	// Dividing sizeX by number of frames in x creates the correct image
